...
struct ImGuiContext;                // Dear ImGui context (opaque structures, unless including imgui_internal.h)
...
namespace ImGui
{
    ...
    // Main
    IMGUI_API ImGuiIO&      GetIO();                                    // access the IO structures (mouse/keyboard/gamepad inputs, time, various configuration options/flags)
    IMGUI_API ImGuiStyle&   GetStyle();                                 // access the Style structures (colors, sizes). Always use PushStyleCol(), PushStyleVar() to modify style mid-frame!
    ...
    IMGUI_API void          ShowStyleEditor(ImGuiStyle* ref = NULL);    // add style editor block (not a window). you can pass in a reference ImGuiStyle structures to compare to, revert to and save to (else it uses the default style)
    ...
    IMGUI_API const ImVec4& GetStyleColorVec4(ImGuiCol idx);                                // retrieve style color as stored in ImGuiStyle structures. use to feed back into PushStyleColor(), otherwise use GetColorU32() to get style color with style alpha baked in.
    ...
    // - You can pass the address of a first float element out of a contiguous structures, e.g. &myvector.x
...
// Enumeration for PushStyleVar() / PopStyleVar() to temporarily modify the ImGuiStyle structures.
...
enum ImGuiStyleVar_
{
    // Enum name --------------------- // Member in ImGuiStyle structures (see ImGuiStyle for descriptions)
...
// [SECTION] ImGuiIO
//-----------------------------------------------------------------------------
// Communicate most settings and inputs/outputs to Dear ImGui using this structures.
...
#else
    void*       _UnusedPadding;                                     // Unused field to keep data structures the same size.
#endif
...
// as this is one of the oldest structures exposed by the library! Basically, ImDrawList == CmdList)
struct ImDrawData
{
...
struct ImFontAtlas
{
    ...
    ImTextureID                 TexID;              // User data to refer to the texture once it has been uploaded to user's graphic systems. It is passed back to you during rendering via the ImDrawCmd structures.
